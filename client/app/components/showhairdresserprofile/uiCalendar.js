...
export const uiCalendar = angular.module('ui.rCalendar', ['ui.rCalendar.tpls'])
                ...
                ctrl._onDataLoaded = function () {
                    ...
                    for (var i = 0; i < len; i += 1) {                        
                        ...
                        while (index < timeDifferenceEnd - eps) {
                            ...
                            if(event.status=='locked'){//check if the day is locked by the hairdresser
                                rows[rowIndex][dayIndex].isLocked=true;
                            }else{
                                rows[rowIndex][dayIndex].isLocked=false;
                            }
...
angular.module("template/rcalendar/calendar.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/rcalendar/calendar.html",
    ...
    "            <button type=\"button\" class=\"btn btn-default btn-nav btn-sm\" ng-click=\"move(-1)\"><i\n" +
...
angular.module("template/rcalendar/month.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/rcalendar/month.html",
    ...
    "                ng-class=\"{'text-center':true, 'monthview-current': dt.current&&!dt.selected&&!dt.hasEvent,'monthview-secondary-with-event': dt.secondary&&dt.hasEvent&&!dt.isLocked, 'monthview-primary-with-event':!dt.secondary&&dt.hasEvent&&!dt.selected&&!dt.isLocked, 'monthview-selected': dt.selected,'month-view-locked':!dt.selected&&dt.hasEvent&&dt.isLocked}\">\n" +
    ...
    "        </tbody>\n" +
    "    </table>\n" +
    "    <div ng-if=\"showEventDetail && displayScrollable\" class=\"event-detail-container\">\n" +
